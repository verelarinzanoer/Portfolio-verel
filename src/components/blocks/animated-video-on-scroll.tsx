...
>(({ style, className, transition, ...props }, ref) => {
  const { scrollYProgress } = useContainerScrollContext();
  const scale = useTransform(scrollYProgress, [0, 0.95], [0.7, 1]);
...
>(
  (
    ...
    const insetY = useTransform(scrollYProgress, [0, 0.95], insetYRange);
    const insetX = useTransform(scrollYProgress, [0, 0.95], insetXRange);
